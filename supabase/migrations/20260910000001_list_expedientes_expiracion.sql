-- ============================================================
-- Estudios con la autorización expirada: la pelota es del gestor
-- ------------------------------------------------------------
-- Flujo §12: "El prospecto no autoriza. El estudio expira transcurrido el
-- plazo definido. El solicitante puede reenviar la solicitud sin costo". La
-- lista no conocía la expiración (solo el detalle la calculaba, en la API):
-- un estudio expirado seguía diciendo "Depende del prospecto" para siempre y
-- nunca aparecía en "Requieren mi acción".
--
-- Cambios, sin tocar tablas ni la firma de la función:
--   * el LATERAL `ev` calcula `expirado` con la misma regla que
--     estudios/expiracion.ts (estado que espera al prospecto + última
--     autorización del titular sin firmar y más vieja que
--     DIAS_EXPIRACION_ESTUDIO, leído de parametros_calibracion, default 15);
--   * (f) en los cuatro predicados de "requiere acción" → depende_de='gestor';
--   * `estudio_vigente.expirado` para que el web pinte el badge "Expirado".
--
-- Misma firma que 20260908000002, así que basta CREATE OR REPLACE.
-- ROLLBACK: reaplicar 20260908000002.
-- ============================================================
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT es.estado::TEXT AS estado, es.resultado::TEXT AS resultado, es.score, es.created_at,
           ...
           (es.estado::TEXT IN ('solicitado','pago_pendiente','pagado','formulario_enviado')
            AND EXISTS (
              SELECT 1 FROM (
