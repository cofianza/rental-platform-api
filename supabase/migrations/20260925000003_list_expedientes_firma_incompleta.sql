-- ============================================================
-- Contratos V3 — Entrega 5 (3 de 3): FIRMA INCOMPLETA en "Requieren mi accion".
-- ------------------------------------------------------------
-- Un contrato V3 cuya firma quedo incompleta (V3 §11.7.4-5) es trabajo de la
-- inmobiliaria: reenviarlo a firma o cancelarlo. Sin esto el estudio sale de la
-- bandeja y nadie lo ve (depende_de queda NULL).
--
-- Unico cambio frente a 20260910000002 (la definicion vigente): la condicion
-- (g), agregada en las CUATRO copias del bloque "requiere accion" — el filtro
-- de la lista, la columna requiere_accion, la rama 'gestor' de depende_de y el
-- filtro del conteo total. Misma firma -> CREATE OR REPLACE.
-- La rama pendiente_firma -> 'prospecto' no cambia: en EN FIRMA la accion es de
-- los firmantes.
--
-- Correr DESPUES de 20260925000001 (el enum trae 'firma_incompleta').
...
-- ============================================================
-- Verificacion manual (no deja rastro). Debe salir el NOTICE del final.
--   BEGIN;
--   DO $v$
--   DECLARE
...
--     UPDATE contratos SET estado = 'firma_incompleta' WHERE id = c;   -- OLD en borrador: el congelamiento no lo revisa
--     SELECT (list_expedientes_with_relations(p_estudio_filtro => 'requiere_accion', p_limit => 1000)->>'total')::INT INTO despues;
--     ASSERT despues = antes + 1, format('FALLA: el total paso de %s a %s (esperado +1)', antes, despues);
--     RAISE NOTICE 'Verificacion E5 (003): el estudio con firma incompleta entra en "Requieren mi accion"';
--   END $v$;
--   ROLLBACK;
-- ============================================================
