...
export const documentoIdParamsSchema = z.object({
  id: z.uuid({ error: 'ID de documento inválido' }),
});
...
export const presignedUrlSchema = z.object({
  ...
  tipo_documento_id: z.uuid({ error: 'ID de tipo de documento inválido' }),
...
export const confirmarSubidaSchema = z.object({
  ...
  tamano_bytes: z.coerce.number().int().positive('Tamaño debe ser positivo'),
...
export const listDocumentosQuerySchema = z.object({
  ...
  tipo_documento_id: z.uuid({ error: 'ID de tipo de documento inválido' }).optional(),
  estado: z.enum(ESTADOS_DOCUMENTO, { error: `Estado inválido. Valores: ${ESTADOS_DOCUMENTO.join(', ')}` }).optional(),
