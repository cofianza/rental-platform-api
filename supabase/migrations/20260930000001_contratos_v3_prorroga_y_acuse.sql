...
--     prorrogó (la marca es además el candado de "una sola vez") y el
--     vencimiento que se le mandó a Auco (allá no se puede mover: la prórroga
--     no lo pasa).
...
--     DROP COLUMN IF EXISTS auco_expira_en,
...
ALTER TABLE public.contrato_v3_sobres
  ...
  ADD COLUMN IF NOT EXISTS auco_expira_en TIMESTAMPTZ,
...
COMMENT ON COLUMN public.contrato_v3_sobres.auco_expira_en IS
  'Adenda 1 (respuesta 10): expiredDate enviado a Auco (lo maximo con la prorroga, tope CRC). La prorroga no lo pasa.';
...
-- Verificacion (solo lectura; 6 filas):
--   SELECT column_name, data_type FROM information_schema.columns
--   WHERE table_schema = 'public' AND table_name = 'contrato_v3_sobres'
--     AND column_name IN ('plazo_prorrogado_en', 'plazo_prorrogado_por', 'auco_expira_en',
