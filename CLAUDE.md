...
**Middleware stack** (order in `app.ts`): helmet → cors → generalLimiter (`RATE_LIMIT_MAX` per minute, default 300) → pino-http (with request ID tracing) → express.json → express.urlencoded → routes → errorHandler.
...
- `src/middleware/rateLimiter.ts` — `generalLimiter` (`RATE_LIMIT_MAX` per minute, default 300) and `authLimiter` (10 req/min, for auth routes).
- `src/modules/` — business domain modules following the pattern: `*.routes.ts` → `*.controller.ts` → `*.service.ts`.
- `src/types/auth.ts` — `UserRole` type (`administrador | operador_analista | gerencia_consulta | propietario | inmobiliaria | solicitante`) and `AuthUser` interface.
...
- **One person, one organization at a time:** accepting/registering an invitation while active in another org is a 409 `YA_PERTENECE_A_OTRA_INMOBILIARIA` (`vincularMiembro`), backed by a partial unique index on `inmobiliaria_miembros(perfil_id) WHERE estado='activo'` (migration 20261001000008, **which must be run**: until then the app-level check can't stop two simultaneous acceptances from leaving someone in two orgs, and a closed org can't be marked `cerrada`; its 23505 becomes the same 409). Accepting never touches the other org: a titular gets 409 `TITULAR_DE_OTRA_INMOBILIARIA` with a message for their case (co-titular: step down to member; team or cartera: transfer titularity or ask Cofianza; sole titular of an empty org: `details.puede_cerrar=true` + the org name). If the membership lookup fails it's a 503 (fail closed). The acceptance UPDATE is conditional (`estado='invitado'` and `perfil_id` null or the same person, so a removed member can be re-invited) and idempotent (0 rows → re-read; already active for that perfil = success). An org is closed only explicitly: `salirDeOrg` by its sole titular when it's empty (no other active members, inmuebles, live estudios, solicitante fichas, unused credits or pending credit purchases) revokes their membership and pending invitations and marks it `estado='cerrada'` (no rows deleted, bitácora `inmobiliaria_cerrada`); it recounts active members after revoking and reopens (409 `INMOBILIARIA_CON_EQUIPO`) if someone got in, and reopening never reactivates someone already active elsewhere nor overwrites a state other than `cerrada`. `listMiembros` returns `puede_cerrar` for that case; co-titulars and titulars with team or cartera still get `TITULAR_NO_PUEDE_SALIR`. An account that isn't an inmobiliaria's (propietario, arrendatario or Cofianza staff) can't join a team: inviting (or re-sending to) its email is a 409 `EMAIL_OTRO_ROL` with one generic message that doesn't say which kind of account it is, the public invitation info returns `cuenta_otro_rol: boolean` for invitations already sent, and both routes go through `invitarMiembroLimiter` (20/hour per user).
...
**`src/lib/tenantScope.ts` is the single source of truth for "what can a user see".** Reuse it; do not re-derive scoping inline, because a second copy drifts and leaks data across agencies. Key exports:
...
- `assertInmuebleAccess` / `assertExpedienteAccess` (and `puedeVerFilaExpediente` for a row already read) → the by-id guards (404) for detail/write endpoints. Same cartera rule as the lists: owner, or member with `miembros_ven_todo` = the whole org; restricted member = only what they created or were assigned. Use them instead of `propietario_id === userId`.
- `perfilEsDuenoDeInmueble(...)` → org-level check only (direct `propietario_id` OR any active member of the owning org, ignoring `miembros_ven_todo`). Never a by-id guard on its own — that let restricted members open, run and pay for their colleagues' estudios by id. Its only caller left, `liberarEstudioConCredito`, runs after `assertExpedienteAccess` in all three entry points (the `liberar-estudio-credito` controller, `cancelarYLiberarCredito`, `iniciarEstudio`).
- `esOwnerDeOrg`, `esMiembroNoOwnerDeOrg`, `esMiembroSoloLectura`, `resolveOrgMemberPerfilIds`, `ensureOrgConOwner`.

**`solo_lectura` write-block lives in `src/middleware/auth.ts`** (the universal authenticated chokepoint): for a `rol='inmobiliaria'` viewer, mutating methods (POST/PUT/PATCH/DELETE) are **denied by default**, with a small allowlist (`/notificaciones`, `/auth`, `/users`, `/inmobiliaria/miembros/salir`, and `/public/invitacion-miembro/:token/aceptar` so accepting another team's invitation gets the service's clear 409 instead of the generic block; the same allowlist skips the incomplete-profile block). Consequence: **a new mutating route is automatically blocked for viewers** — if a viewer legitimately needs it, add the path to `viewerPuedeMutar()`. This is the deliberate safe bias; don't replace it with per-route checks.
