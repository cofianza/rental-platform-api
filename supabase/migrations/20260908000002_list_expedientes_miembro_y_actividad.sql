-- ============================================================
-- Filtro por miembro responsable + orden por última actividad
-- ------------------------------------------------------------
-- Dos huecos del listado de estudios:
--   1. El select "Todos los responsables" de la vista fusionada nunca filtraba
--      nada: el único parámetro que aceptaba la función era p_analista_id (el
--      analista interno de Cofianza), no el miembro del equipo de la
--      inmobiliaria. Se añade p_miembro_responsable_id.
--   2. La columna "Última actividad" mostraba los días sin movimiento pero el
--      encabezado solo podía ordenar por created_at. Se acepta updated_at.
--
-- Añadir un parámetro cambia la firma, así que hay que DROP + CREATE (un
-- CREATE OR REPLACE crearía una sobrecarga y PostgREST no sabría cuál llamar).
-- La API manda p_miembro_responsable_id solo cuando viene con valor, así que
-- un deploy anterior a esta migración sigue funcionando.
--
-- Base: 20260908000001 (requiere_accion). Sin cambios de tablas ni columnas.
-- ROLLBACK: reaplicar 20260908000001.
-- ============================================================

DROP FUNCTION IF EXISTS public.list_expedientes_with_relations(
  text, text[], uuid, uuid, timestamptz, timestamptz, text, text, integer, integer, text, uuid[]
);
