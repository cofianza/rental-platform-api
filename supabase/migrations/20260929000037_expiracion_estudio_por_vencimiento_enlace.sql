-- ============================================================
-- Expiración del estudio anclada al vencimiento guardado del enlace
-- (revisión 2026-09-23, hueco4-4).
--
-- El enlace del prospecto guarda su vencimiento al crearse
-- (autorizaciones_habeas_data.token_expiracion = envío + DIAS_EXPIRACION_ESTUDIO
-- de ese momento) y se valida contra ese valor. Pero el veredicto «expirado» del
-- listado se recalculaba como created_at + el DIAS_EXPIRACION_ESTUDIO ACTUAL: si
-- Gerencia subía el plazo, el estudio seguía «esperando al prospecto» con el
-- enlace ya muerto; si lo bajaba, salía «expirado» con el enlace vivo.
--
-- Ahora vence cuando vence el enlace; el plazo de calibración solo es respaldo
-- para filas sin token_expiracion. Mismo criterio que la API
-- (estudios.service.ts, veredictoExpiracion).
--
-- Único cambio frente a 20260927000002 (cuerpo verificado contra producción por
-- md5 el 2026-09-23): las dos copias del subselect `ult` traen token_expiracion
-- y la condición usa COALESCE. El resto es idéntico.
-- ============================================================
...
--   SELECT (length(d) - length(replace(d, 'COALESCE(ult.token_expiracion', ''))) / length('COALESCE(ult.token_expiracion')
--   FROM (SELECT pg_get_functiondef('public.list_expedientes_with_relations(text,text[],uuid,uuid,timestamptz,timestamptz,text,text,integer,integer,text,uuid[],uuid)'::regprocedure) AS d) f;
