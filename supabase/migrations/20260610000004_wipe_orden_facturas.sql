-- ============================================================
-- TEMPORAL — fix del wipe de QA (orden de borrado): los FK de Postgres NO son
-- deferibles por defecto, así que `SET CONSTRAINTS ALL DEFERRED` era un no-op
-- y el wipe abortaba por FK en cuanto existiera una factura (facturas
-- referencia pagos/compras y se borraba DESPUÉS de ellas). Se borra facturas
-- PRIMERO y se elimina el SET CONSTRAINTS engañoso.
--
-- Mismo contrato que 20260610000002/3: borra SOLO datos transaccionales de
-- prueba; conserva TODAS las cuentas y la configuración. WHERE true por
-- pg-safeupdate. ELIMINAR junto con el módulo admin-tools antes de producción.
-- ============================================================
...
COMMENT ON FUNCTION fn_wipe_test_data() IS
  'TEMPORAL: borra datos transaccionales de prueba CONSERVANDO todas las cuentas (facturas primero por FK; WHERE true por pg-safeupdate). ELIMINAR antes de produccion.';
