...
- **One person, one organization at a time:** accepting/registering an invitation while active in another org is a 409 `YA_PERTENECE_A_OTRA_INMOBILIARIA` (`vincularMiembro`), backed by a partial unique index on `inmobiliaria_miembros(perfil_id) WHERE estado='activo'` (migration 20261001000008; its 23505 becomes the same 409). A titular can't leave their own org, so if they're its only titular and it's empty (no other active members, inmuebles, live estudios or unused credits) accepting closes it (membership + pending invitations `revocado`, org `estado='cerrada'`, bitácora `inmobiliaria_cerrada`, no rows deleted); otherwise 409 `TITULAR_DE_OTRA_INMOBILIARIA`. The acceptance UPDATE is conditional (`estado='invitado'` and `perfil_id` null or the same person, so a removed member can be re-invited). An account that isn't an inmobiliaria's (propietario, arrendatario or Cofianza staff) can't join a team: inviting (or re-sending to) its email is a 409 `EMAIL_OTRO_ROL` with one generic message that doesn't say which kind of account it is, the public invitation info returns `cuenta_otro_rol: boolean` for invitations already sent, and both routes go through `invitarMiembroLimiter` (20/hour per user).
