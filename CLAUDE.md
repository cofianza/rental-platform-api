...
- **One person, one organization at a time:** accepting/registering an invitation while active in another org is a 409 `YA_PERTENECE_A_OTRA_INMOBILIARIA` (`vincularMiembro`), backed by a partial unique index on `inmobiliaria_miembros(perfil_id) WHERE estado='activo'` (migration 20261001000008). A propietario/arrendatario account can't join a team: inviting (or re-sending to) its email is a 409 `EMAIL_OTRO_ROL`, and the public invitation info returns `cuenta_otro_rol` (`'propietario_o_arrendatario'` | `'interna'` for Cofianza staff | null) for invitations already sent.
