...
- `perfilEsDuenoDeInmueble(...)` → org-level check only (direct `propietario_id` OR any active member of the owning org, ignoring `miembros_ven_todo`). Only for guards already behind `assertExpedienteAccess` or org-wide by design (liberar con crédito); never as the by-id guard of an inmueble — that let restricted members open their colleagues' portfolio by link.
