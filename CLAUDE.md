...
- **One person, one organization at a time:** accepting/registering an invitation while active in another org is a 409 `YA_PERTENECE_A_OTRA_INMOBILIARIA` (`vincularMiembro`), backed by a partial unique index on `inmobiliaria_miembros(perfil_id) WHERE estado='activo'` (migration 20261001000008).
