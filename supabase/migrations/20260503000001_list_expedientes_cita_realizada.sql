-- ============================================================
-- Agregar `cita_realizada` al RPC list_expedientes_with_relations.
--
-- Contexto: el frontend muestra un indicador del paso del proceso
-- (Solicitud → Cita previa → Estudio → Aprobacion → Contrato → Firma → Listo)
-- en la lista de expedientes. Sin este flag no se puede distinguir si un
-- expediente en estado 'borrador' esta en paso 1 (Solicitud / Cita previa
-- pendiente) o ya en paso 3 (Estudio, post-cita).
--
-- Implementacion: subquery EXISTS sobre citas con estado='realizada'. Usa
-- el indice existente sobre citas.expediente_id (FK), asi que el costo es
-- O(1) por fila listada.
-- ============================================================
...
BEGIN
  ...
  FROM (
    SELECT json_build_object(
      ...
      -- TRUE si existe al menos una cita de este expediente con estado 'realizada'.
      -- Lo usa el frontend para distinguir paso "Cita previa" vs "Estudio" en
      -- expedientes en estado 'borrador'. EXISTS corta tan pronto encuentra una
      -- fila → costo O(1) sobre el indice de citas(expediente_id).
