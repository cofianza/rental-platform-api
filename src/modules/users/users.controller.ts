...
export async function update(req: Request, res: Response) {
  ...
  const user = await usersService.updateUser(id, input, req.user!.id);
  sendSuccess(res, user);
}

export async function deactivate(req: Request, res: Response) {
  const { id } = req.params as unknown as UserIdParams;
  const user = await usersService.deactivateUser(id, req.user!.id);
  sendSuccess(res, user);
}

export async function activate(req: Request, res: Response) {
  const { id } = req.params as unknown as UserIdParams;
  const user = await usersService.activateUser(id, req.user!.id);
  sendSuccess(res, user);
}
